...
local cpuTableOverrides = [
  ...
  { matcher: { id: 'byName', options: 'Limit %' }, properties: [
    ...
    { id: 'mappings', value: [
      { type: 'special', options: { match: 'null', result: { text: 'N/A', color: 'text' } } },
      { type: 'special', options: { match: 'nan', result: { text: 'N/A', color: 'text' } } },
      { type: 'special', options: { match: 'null+nan', result: { text: 'N/A', color: 'text' } } },
    ] },
  ] },
  ...
  panels: [
    ...
    // CPU Table
    {
      ...
      targets: [
        ...
        { datasource: { type: 'prometheus', uid: '${datasource}' }, expr: 'avg_over_time((sum(rate(container_cpu_usage_seconds_total{%s}[5m])) by (namespace, pod))[$__range:5m]) * 1000' % selector, format: 'table', instant: true, refId: 'avg' },
        { datasource: { type: 'prometheus', uid: '${datasource}' }, expr: 'quantile_over_time(0.95, (sum(rate(container_cpu_usage_seconds_total{%s}[5m])) by (namespace, pod))[$__range:5m]) * 1000' % selector, format: 'table', instant: true, refId: 'p95' },
        { datasource: { type: 'prometheus', uid: '${datasource}' }, expr: 'clamp_min(ceil(quantile_over_time(0.95, (sum(rate(container_cpu_usage_seconds_total{%s}[5m])) by (namespace, pod))[$__range:5m]) * 1.2 * 100) * 10, 10)' % selector, format: 'table', instant: true, refId: 'recommended' },
      ],
    ...
    // Memory Table
    {
      ...
      targets: [
        ...
        { datasource: { type: 'prometheus', uid: '${datasource}' }, expr: 'avg_over_time((sum(container_memory_working_set_bytes{%s}) by (namespace, pod))[$__range:5m]) / 1048576' % selector, format: 'table', instant: true, refId: 'avg' },
        { datasource: { type: 'prometheus', uid: '${datasource}' }, expr: 'quantile_over_time(0.95, (sum(container_memory_working_set_bytes{%s}) by (namespace, pod))[$__range:5m]) / 1048576' % selector, format: 'table', instant: true, refId: 'p95' },
        { datasource: { type: 'prometheus', uid: '${datasource}' }, expr: 'clamp_min(ceil(quantile_over_time(0.95, (sum(container_memory_working_set_bytes{%s}) by (namespace, pod))[$__range:5m]) * 1.2 / 67108864) * 64, 64)' % selector, format: 'table', instant: true, refId: 'recommended' },
      ],
